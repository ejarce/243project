...
```{r}
library(MASS)
LDA1 <- lda(DegreeField ~ .-IDnum-GSSyear, data = GSS3)
```
...
```{r}
QDA1 <- qda(STEM ~ .-IDnum-GSSyear-X1-DegreeField, data = train)
```

### SVM
```{r}
library(e1071)
SVM <- svm(STEM ~ .-IDnum-GSSyear-X1-DegreeField, data = train)
summary(SVM)
ypred <- predict(SVM, test)

table(predict = ypred, truth = test$STEM)
```
